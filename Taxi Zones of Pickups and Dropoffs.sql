...
where left(ltrim(rtrim([r].[originating_base_num])),1)='B'
	and left(ltrim(rtrim([r].[dispatching_base_num])),1)='B'
	and [r].[request_outcome] <> 3 
	and isdate([R].[request_datetime])=1
	and isdate([T].[on_scene_datetime])=1
	and len([T].[hvfhs_license_num]) > 1
	and len([R].[hvfhs_license_num]) > 1
